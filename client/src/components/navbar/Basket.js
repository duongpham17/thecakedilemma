        ...
        <div className={`basket-nested-container ${!basket || basket.length === 0 ? "no-border" : ""}`}>
            {!basket ? "" : basket.slice(0, 5).map((el) => 
                <div className="basket-content" key={el.unique}>
                    <li className="area-title">{el.title.length > 15 ? `${el.title.slice(0, 15)}...` : el.title}</li> <li className="area-total">£{Number(el.total).toFixed(2)}</li>
            ...
            <div className="total-content">
                ...
                <Fragment>
                    ...
                    <li className="area-more-total">£{Number(slice).toFixed(2)}</li>
                </Fragment>
                : ""}
